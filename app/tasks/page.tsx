          ...
          {loading ? (
            ...
            <ul className="space-y-4">
              {tasks.map(task => (
                <li 
                  key={task.id} 
                  className="group flex flex-col sm:flex-row sm:items-center justify-between p-5 rounded-2xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all gap-4"
                >
                  ...
                  {editingId === task.id ? (
                    <div className="flex-1 flex items-center gap-3">
                      <input 
                        ...
                        className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400"
                        autoFocus
                      />
                      <button 
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-slate-700 mb-1">{task.title}</h3>
                      <p className="text-sm text-slate-400">
                        创建时间: {format(new Date(task.createdAt), "yyyy年MM月dd日 HH:mm")}
                      </p>
                    </div>
                  )}
